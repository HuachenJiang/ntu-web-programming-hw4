...
- `services/`: Axios API client、API 请求封装与本地 auth storage
...
- `apiClient` 统一负责 Axios `baseURL`、JWT Authorization header 注入与后端错误格式转换
...
6. 后端完成验证、权限检查与资料写入
7. 记录列表、详情与编辑页统一经由 Axios service 从 backend 读取或更新资料
